...
QuicCryptoServerStreamBase* QuicServerSession::GetMutableCryptoStream() {
  return crypto_stream_.get();
}

const QuicCryptoServerStreamBase* QuicServerSession::GetCryptoStream()
    const {
  return crypto_stream_.get();
}

int32_t QuicServerSession::BandwidthToCachedParameterBytesPerSecond(
...
QuicStream* QuicServerSession::CreateIncomingDynamicStream(
    QuicStreamId id) {
  ...
  QuicStream* stream =
      new QuicServerStream(id, this);
  ActivateStream(QuicWrapUnique(stream));
  return stream;
}

QuicStream* QuicServerSession::CreateOutgoingDynamicStream(
    SpdyPriority priority) {
  if (!ShouldCreateOutgoingDynamicStream()) {
    return nullptr;
  }

  QuicStream* stream = new QuicServerStream(
...
std::unique_ptr<QuicStream> QuicServerSession::CreateStream(
    QuicStreamId id) {
  return QuicMakeUnique<QuicServerStream>(id, this);
}
}  // namespace net
