...
int QuicDispatcher::GetSession()
{
   return session_map_.size();
}

int QuicDispatcher::GetSteam()
{
/*	SessionMap::iterator it = session_map_.find(current_connection_id_);
        QuicServerSessionBase* session = it->second.get();
        QuicStream* steam = session->GetOrCreateStream(7);
        steam-> WriteOrBufferData("123", true, nullptr);*/
        return 1;
}
