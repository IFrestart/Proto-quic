...
QuicClientMessageLoop::QuicClientMessageLoop(
...
bool QuicClientMessageLoop::CreateUDPSocketAndBind(
...
void QuicClientMessageLoop::CleanUpAllUDPSockets() {
  client_->reset_writer();
  packet_reader_.reset();
  packet_reader_started_ = false;
}

void QuicClientMessageLoop::StartPacketReaderIfNotStarted() {
...
void QuicClientMessageLoop::RunEventLoop() {
  StartPacketReaderIfNotStarted();
  base::RunLoop().RunUntilIdle();
}

QuicPacketWriter*
QuicClientMessageLoop::CreateQuicPacketWriter() {
  return new QuicChromiumPacketWriter(socket_.get());
}

void QuicClientMessageLoop::OnReadError(
    int result,
...
QuicSocketAddress QuicClientMessageLoop::GetLatestClientAddress()
    const {
  return client_address_;
}

bool QuicClientMessageLoop::OnPacket(
